...
mod tests {
    ...
    fn contains_needle_tests() {
		...
		//on String - match
        let res = "The quick brown fox jumps over the lazy dog".to_string().contains_needle("fox");
        assert_eq!(res, true);
		
		//on String - no match
        let res = "The quick brown fox jumps over the lazy dog".to_string().contains_needle("cat");
        assert_eq!(res, false);
